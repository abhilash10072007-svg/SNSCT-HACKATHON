...
// @POST /api/doubts/:id/ai-answer - Manually trigger AI answer (for testing)
router.post('/:id/ai-answer', protect, async (req, res) => {
  try {
    const doubt = await Doubt.findById(req.params.id);
    if (!doubt) {
    ...
    if (doubt.aiAnswered) {
      return res.status(400).json({ success: false, message: 'Already AI answered' });
    }
    ...
    doubt.answers.push({
      content: `🤖 **AI Generated Answer**\n\n${aiContent}\n\n---\n*This answer was automatically generated by SmartDoubt AI.*`,
      author: aiUser._id,
      isAnonymous: false,
      upvoteCount: 0
    });
    ...
    res.json({ success: true, message: 'AI answer generated successfully!' });
  } catch (error) {
