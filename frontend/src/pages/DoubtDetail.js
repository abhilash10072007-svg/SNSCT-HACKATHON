...
import { ChevronUp, CheckCircle, Eye, Clock, Tag, MessageSquare, Send, ArrowLeft } from 'lucide-react';
...
export default function DoubtDetail() {
  ...
  const handleUpvoteDoubt = async () => {
    try {
      ...
      setDoubt(d => ({ ...d, upvoteCount: res.data.upvoteCount,
        upvotes: res.data.upvoted ? [...(d.upvotes || []), user._id] : (d.upvotes || []).filter(uid => uid !== user._id) }));
    ...
    try {
      ...
      setDoubt(d => ({
        ...
        answers: d.answers.map(a => a._id === answerId ? {
          ...
          upvotes: res.data.upvoted ? [...(a.upvotes || []), user._id] : (a.upvotes || []).filter(uid => uid !== user._id)
        } : a)
      }));
    } catch { toast.error('Failed to upvote answer'); }
  };
  ...
  const handleAcceptAnswer = async (answerId) => {
    try {
      ...
      fetchDoubt();
    } catch (err) { toast.error(err.response?.data?.message || 'Failed to accept answer'); }
  };

  const handleSubmitAnswer = async (e) => {
    e.preventDefault();
    if (!answerText.trim() || answerText.length < 10) return toast.error('Answer must be at least 10 characters');
    setSubmitting(true);
    try {
  ...
  return (
    <div className="page-container">
      <button className="btn btn-ghost btn-sm" onClick={() => navigate(-1)} style={{ marginBottom: 16, display: 'flex', alignItems: 'center', gap: 6 }}>
      ...
      <div className="two-col">
        <div>
          ...
          <div className="doubt-detail-card">
            ...
            <h1 style={{ fontSize: 22, fontWeight: 700, lineHeight: 1.4, marginBottom: 16 }}>{doubt.title}</h1>

            <div className="markdown-content" style={{ marginBottom: 24 }}>{doubt.content}</div>
              ...
              <div key={answer._id} className={`answer-card ${answer.isAccepted ? 'accepted' : ''}`}>
                {answer.isAccepted && (
                  <div style={{ marginBottom: 12 }}>
                    <span className="accepted-badge"><CheckCircle size={14} /> Accepted Answer</span>
                  </div>
                )}
                <div className="markdown-content" style={{ marginBottom: 16 }}>{answer.content}</div>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                    ...
                    {isAuthor && !answer.isAccepted && doubt.status !== 'solved' && (
                      <button
                        className="btn btn-secondary btn-sm"
                  ...
                  <div className="user-info">
                    ...
                    <div>
                      <div className="user-name">{answer.author?.name || 'Anonymous'}</div>
                      <div className="user-meta">{formatDistanceToNow(new Date(answer.createdAt), { addSuffix: true })}</div>
                    </div>
                  </div>
                </div>
              </div>
          ...
          {doubt.status === 'closed' && (
            <div style={{ background: 'var(--bg-hover)', border: '1px solid var(--border)', borderRadius: 12, padding: 20, marginTop: 16, textAlign: 'center', color: 'var(--text-muted)' }}>
        ...
        <div>
          <div className="card">
            <h4 style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-muted)', marginBottom: 14, textTransform: 'uppercase', letterSpacing: 1 }}>About this doubt</h4>
          ...
          {doubt.tags?.length > 0 && (
            <div className="card" style={{ marginTop: 12 }}>
              <h4 style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-muted)', marginBottom: 12, textTransform: 'uppercase', letterSpacing: 1 }}>Tags</h4>
